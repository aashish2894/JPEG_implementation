...
for i = 1:size(Y_AC,1)
    ...
    if Y_AC(i,1) <= 15
        ...
        runlength = strcat(dec2hex(Y_AC(i,1)),dec2hex(Y_AC(i,2)));
        i_AC = find(ismember(Y_AC_run_length,lower(runlength)));
        Y_AC_huffman_code = strcat(Y_AC_huffman_code,Y_AC_code(i_AC));
        Y_AC_huffman_code = strcat(Y_AC_huffman_code,binary_value);
    else
        i_f0 = find(ismember(Y_AC_run_length,'f0')); % if the number of consecutive zeros are more than 16 than make an element (15,0) and then continue normally
        Y_AC_huffman_code = strcat(Y_AC_huffman_code,Y_AC_code(i_f0));
        binary_value = dec2bin(abs(Y_AC(i,3)));
        if Y_AC(i,3) < 0 
            for k = 1:length(binary_value)
                if binary_value(k) == '0'
        ...
        runlength = strcat(dec2hex(Y_AC(i,1)-16),dec2hex(Y_AC(i,2)));
        i_AC = find(ismember(Y_AC_run_length,lower(runlength)));
        Y_AC_huffman_code = strcat(Y_AC_huffman_code,Y_AC_code(i_AC));
        Y_AC_huffman_code = strcat(Y_AC_huffman_code,binary_value);
    end    
...
% Y DC
Y_DC_huffman_code = dec2bin(abs(Y_DC)); % dec2bin return the binary number in the form of string
for i = 1:length(Y_DC_length)
    if Y_DC_length(i)==length(Y_DC_huffman_code)
        i_DC = i;   % find the index in Y_DC_length which has the same number of bits as Y_DC
        break;
    end
end
if Y_DC < 0   % if the number is negative than the store the number in 1's complement form
    for i = 1:length(Y_DC_huffman_code)
        if Y_DC_huffman_code(i)=='0'
            Y_DC_huffman_code(i)='1';
        else
            Y_DC_huffman_code(i)='0';
        end
    end
end
Y_DC_huffman_code = strcat(Y_DC_code(i_DC),Y_DC_huffman_code);
if Y_DC==0
    Y_DC_huffman_code='00';
end
Y_DC_AC = strcat(Y_DC_huffman_code,Y_AC_huffman_code);
final_output = strcat(final_output,Y_DC_AC);
end
